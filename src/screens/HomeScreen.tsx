...
// theme: Nuestro diccionario de colores y diseños que vimos antes.
import { theme } from "../styles/theme";
// useStore: Nuestro "cerebro" global que guarda datos de la app (créditos, rutinas, etc.).
import { useStore, Emotion, calculateStreak } from "../store/useStore";
// lucide-react-native: Una librería para poner íconos bonitos en la app.
import {
    CheckCircle2,
    Circle,
    Flame,
...
// Esta función se ejecuta cada vez que algo visual tiene que actualizarse.
export default function HomeScreen() {
    // 2. LECTURA DEL ESTADO GLOBAL
    // Usamos el hook `useStore()` para extraer las variables y funciones que necesitamos.
    // Al hacerlo así, si 'creditosDisponibles' cambia, esta pantalla se actualiza sola automáticamente.
    const {
        dailyLogs,
        updateDailyLog,
        dailyPlans,
    ...
    const todayStr = useStore((state) => state.currentDate);
    ...
    // 3. ESTADO LOCAL (solo vive en esta pantalla)
    // useState: Guarda un valor (barWidth) y nos da una función para cambiarlo (setBarWidth).
    const [barWidth, setBarWidth] = useState(0);
    // useRef: Guarda un valor que persiste entre recargas de la pantalla (ideal para animaciones).
    ...
    // useEffect se ejecuta automáticamente cuando la pantalla arranca o cuando las variables
    // en su lista (como [todayStr, todaysPlan, generateDailyPlan]) cambian.
    // Aquí le decimos: "Si no hay plan para hoy, genéralo por favor".
    useEffect(() => {
        if (!todaysPlan) {
    ...
    const handleAddPagesHome = () => {
        ...
        const today = useStore.getState().currentDate;
        try {
            addStudyLog({
                date: today,
                type: "reading",
    ...
    const renderTaskToggle = (
        title: string,
        field: keyof typeof todaysLog,
        domain: "Fitness" | "Ingles" | "Lectura",
        level: number,
        streak?: number,
    ) => (
            ...
            >
                ...
                <View style={{ flex: 1, marginLeft: theme.spacing.md }}>
                    <Text
                        style={[
                            styles.taskText,
                            (todaysLog[field] as any) && styles.taskTextDone,
                        ]}
                    >
                        {title}
                    </Text>
    ...
    // 5. RENDERIZADO VISUAL (Lo que el usuario ve)
    // La palabra 'return' marca el final lógico de la función. Todo lo que va adentro de los
    // paréntesis () es JSX: parece HTML, pero usa componentes móviles verdaderos.
    return (
        // <View> es como un "div" en web o una caja vacía en la pantalla.
        <View style={{ flex: 1 }}>
            {/* <ScrollView> es una caja manejable que permite hacer "scroll" (deslizar) */}
            <ScrollView
            ...
            >
                <View style={styles.header}>
                    ...
                    >
                        ...
                        />
                        <Text style={[styles.date, { flex: 1, textAlign: "center" }]}>
                            {new Date(todayStr + "T12:00:00Z").toLocaleDateString("es-ES", {
                                weekday: "long",
                                day: "numeric",
                                month: "long",
                            })}
                        ...
                        <View
                            style={{ alignItems: "flex-end", marginLeft: theme.spacing.md }}
                        >
                            <Animated.Text
                                style={[
                                    styles.date,
                            ...
                            {creditDeltaText && (
                                ...
                                >
                                    <View style={styles.deltaBadge}>
                                        <Text style={styles.deltaText}>{creditDeltaText}</Text>
                                    </View>
                                </Animated.View>
                            )}
                        </View>
                ...
                <View style={styles.card}>
                    ...
                    <View style={styles.buttonRow}>
                        <TouchableOpacity
                            style={[
                                styles.btn,
                                todaysLog.sugarFree === false && styles.btnActiveNo,
                            ]}
                        ...
                        >
                            <Text
                                style={[
                                    styles.btnText,
                                    todaysLog.sugarFree === false && styles.btnTextActive,
                                ]}
                            >
                                No, fui fuerte
                            </Text>
                        ...
                        <TouchableOpacity
                            style={[
                                styles.btn,
                                todaysLog.sugarFree === true && styles.btnActiveYes,
                            ]}
                        ...
                        >
                            <Text
                                style={[
                                    styles.btnText,
                                    todaysLog.sugarFree === true && styles.btnTextActive,
                                ]}
                            >
                                Sí, consumí
                            </Text>
                    ...
                    <View style={{ marginTop: 16 }}>{/* level progress removed */}</View>
                </View>
                        ...
                        <>
                            ...
                            <View style={styles.missionsContainer}>
                                ...
                                <View style={styles.missionCard}>
                                    ...
                                    {(() => {
                                        const pagesToday = (studyLogs || [])
                                            .filter(
                                                (s) => s.date === todayStr && s.type === "reading",
                                            )
                                            .reduce((sum, s) => sum + (s.pagesRead || 0), 0);
                                        return (
                                            <>
                                                <Text style={styles.missionText}>
                                                    {todaysPlan.readingPlan || "Lee hoy"}
                                                </Text>
                                                <View
                                                    style={{
                                                        flexDirection: "row",
                                    ...
                                    {(() => {
                                        ...
                                        return (
                                            <>
                                                ...
                                                >
                                                    Sesión: {totalFocus} min
                                                </Text>
                                                <TouchableOpacity
                                                    style={[
                                                        styles.smallActionBtn,
                                                        todaysLog.english && styles.smallActionBtnDone,
                                                    ]}
                                                    onPress={() => {
                                                        ...
                                                        if (newStatus && !todaysLog.english) {
                                                            try {
                                                                const plan =
                                                                    (dailyPlans && dailyPlans[todayStr]) || null;
            ...
            {snackMsg && (
                <Animated.View
                    style={[styles.snack, { transform: [{ translateY: snackAnim }] }]}
                >
